...
function traceRestifyWrapper(traceHandle, origFn) {
    return function _traceWrappedFn(opts, body, cb) {
        var self = this;
        var args = [opts];
	var span;
        ...
        args.push(function _traceWrappedCb(err, _, res) {
    ...
    return function _handleMaker(name) {
        ...
        if (typeof(opts.appHandle[name].client) === 'object'
            ...
            if (['JsonClient', 'RestifyClient'].indexOf(constructor) !== -1) {
                ...
                handle.client.read = traceRestifyWrapper(req.traceHandle,
                    opts.appHandle[name].client.read);
                handle.client.write = traceRestifyWrapper(req.traceHandle,
                    opts.appHandle[name].client.write);
            } else {
